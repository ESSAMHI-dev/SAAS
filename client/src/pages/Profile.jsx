...
export default function ProfilePage() {
  ...
  const handleImageChange = async (e) => {
    ...
    try {
      ...
      if (res.data.success) setFormData((prev) => ({ ...prev, imageUrl: res.data.imageUrl }));
    } catch (err) {
      console.error(err);
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      setLoading(true);
      await axios.delete("/api/user/profile", { headers: { Authorization: `Bearer ${token}` } });
      ...
      <div className="bg-white p-8 rounded-lg shadow-md w-full max-w-md relative">
        ...
        <div className="relative items-center">
          ...
          <div
            ...
            className="absolute bottom-2 left-55 bg-gray-800 text-white p-1 rounded-full cursor-pointer"
          >
        ...
        <div className="flex flex-col gap-2">
          <button onClick={() => setIsEditing(true)} className="bg-blue-600 text-white py-2 rounded-full cursor-pointer">
            Edit Profile
          </button>
          <button onClick={handleDeleteAccount} className="bg-red-600 text-white py-2 rounded-full cursor-pointer">
            Delete Account
          </button>
          <button
            onClick={() => {
